...
if args.type == 1: QA_or_QH = 'simple_nfp3'
...
use_joaquim_coils = False
MAXITER_stage_1 = 10
MAXITER_stage_2 = 1000
MAXITER_single_stage = 15
MAXFEV_single_stage  = 23
LENGTH_THRESHOLD = 1.3
# max_mode_array = [1]*3 + [2]*2 + [3]*0 + [4]*0 + [5]*0 + [6]*0
max_mode_array = [2,1]*2
nmodes_coils = 6
aspect_ratio_target = 6
JACOBIAN_THRESHOLD = 500
aspect_ratio_weight = 5e-1
min_iota = 0.41
iota_min_QA = min_iota
iota_min_QH = min_iota
maxmodes_mpol_mapping = {1: 5, 2: 5, 3: 5, 4: 6, 5: 6, 6: 7}
coils_objective_weight = 2e+5
CC_THRESHOLD = 0.04
CURVATURE_THRESHOLD = 100
MSC_THRESHOLD = 100
quasisymmetry_weight = 1e-0
vmec_input_filename = os.path.join(parent_path, 'input.'+ QA_or_QH)
nphi_VMEC = 140# if use_extra_coils else 64
ntheta_VMEC = 32
ftol = 1e-3
diff_method = "forward"
R0 = 0.411
...
elongation_weight = 1
nquadpoints = 120
directory = f'optimization_{QA_or_QH}_order{nmodes_coils}_length{LENGTH_THRESHOLD}_cc{CC_THRESHOLD}_curvature{CURVATURE_THRESHOLD}_msc{MSC_THRESHOLD}_mirror{maximum_mirror}'
...
vmec = Vmec(vmec_input_filename, mpi=mpi, verbose=vmec_verbose, nphi=nphi_VMEC, ntheta=ntheta_VMEC, range_surface='full torus')
surf = vmec.boundary
...
##########################################################################################
#Stage 2
if use_joaquim_coils:
    loaded_coils = load(os.path.join(parent_path, 'circurves_opt.json'))
    base_curves=[loaded_coils[0], loaded_coils[1]]
    for cur in base_curves:
        cur.fix_all()
        top_indices = np.argsort(cur.full_x)[-nmodes_coils:]
        result_array = np.full_like(cur.full_x, False)
        result_array[top_indices] = True
        cur.unfix(result_array.astype(bool))
    base_currents = [Current(1) * 1e5 for c in base_curves]
    coils = coils_via_symmetries(base_curves, base_currents, surf.nfp, stellsym=False)
else:
    ncoils = 2
    if planar_coils:
        base_curves = create_equally_spaced_planar_curves(ncoils, surf.nfp, stellsym=False, R0=R0, R1=R1, order=nmodes_coils, numquadpoints=nquadpoints)
    else:
        base_curves = create_equally_spaced_curves(ncoils, surf.nfp, stellsym=False, R0=R0, R1=R1, order=nmodes_coils, numquadpoints=nquadpoints)
...
if use_extra_coils:
    ...
    for count, (radius1, center1, y1) in enumerate(zip(radius1_array, center1_array, y1_array)):
        ...
        base_curves += [new_base_curve]
        coils += coils_via_symmetries([new_base_curve], [new_base_current], surf.nfp, stellsym=True)
...
if comm_world.rank == 0:
    ...
    curves_to_vtk(base_curves, os.path.join(coils_results_path, "base_curves_init"))
...
for iteration, max_mode in enumerate(max_mode_array):
    ...
    if comm_world.rank == 0:
        ...
        #     res = minimize(fun_coils, dofs[:-number_vmec_dofs]*1.05, jac=True, args=({'Nfeval': 0}), method='L-BFGS-B', options={'maxiter': MAXITER_stage_2, 'maxcor': 300}, tol=1e-9)
        #     dofs[:-number_vmec_dofs] = res.x
        #     res = minimize(fun_coils, dofs[:-number_vmec_dofs]*1.02, jac=True, args=({'Nfeval': 0}), method='L-BFGS-B', options={'maxiter': MAXITER_stage_2, 'maxcor': 300}, tol=1e-9)
        #     dofs[:-number_vmec_dofs] = res.x
