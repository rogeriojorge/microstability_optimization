...
results_folder = f'optimization_QA_asymcoils_QAlinked'
...
sc_fieldline = SurfaceClassifier(surf, h=0.05*R_axis, p=2)
...
if interpolate_field:
    ...
    def skip(rs, phis, zs):
        ...
        skip = list((dists < -0.1*R_axis*2.0).flatten())
        # skip = [False]*len(skip)
