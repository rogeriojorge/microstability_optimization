...
CONFIG = {
    ...
    4: {
        ...
        "output_dir": 'nfp3_QA',
        "params": { 'nphi': 51,'nlambda': 15,'nperiod': 2.5,'nstep': 150,'dt': 0.4,
                    'aky_min': 0.3,'aky_max': 3.0,'naky': 6,'LN': 1.0,'LT': 3.0,
                    's_radius': 0.25,'alpha_fieldline': 0,'ngauss': 3,'negrid': 8,'vnewk': 0.01
                  },
        "aspect_ratio_target": 7,
...
os.chdir(OUT_DIR)
vmec = Vmec(filename, verbose=False, mpi=None)
...
for max_mode in max_modes:
    ...
    if optimizer == 'dual_annealing':
        ...
        pprint(f' Solving with diff_rel_step={diff_rel_step} and diff_abs_step={diff_abs_step}')
        ...
        if perform_extra_solve:
            pprint(f' Extra solve with diff_rel_step={diff_rel_step} and diff_abs_step={diff_abs_step}')
            least_squares_mpi_solve(prob, mpi, grad=True, rel_step=diff_rel_step/10, abs_step=diff_abs_step/10, max_nfev=MAXITER, ftol=ftol)#, diff_method=diff_method, method=local_optimization_method)
    else: pprint('Optimizer not available')
    ######################################
    try: 
