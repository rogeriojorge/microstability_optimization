...
beta = 2.5 #%
ne = ProfilePolynomial(2.2e20 * np.array([1, 0, 0, 0, 0, -1.0]))
Te = ProfilePolynomial(10.0e3 * np.array([1, -1.0]))
...
def minor_radius_objective(vmec):
    vmec.run()
    return vmec.wout.Aminor_p

def volavgB_objective(vmec):
    vmec.run()
    return vmec.wout.volavgB

def DMerc_min_objective(vmec):
    vmec.run()
    return np.min(vmec.wout.DMerc[4:])

def magnetic_well_objective(vmec):
    vmec.run()
    return vmec.vacuum_well()
...
DMerc_min_optimizable = make_optimizable(DMerc_min_objective, vmec)
penalty_DMerc=QuadraticPenalty(DMerc_min_optimizable, 0.0, "min")
magnetic_well_optimizable = make_optimizable(magnetic_well_objective, vmec)
penalty_magnetic_well=QuadraticPenalty(magnetic_well_optimizable, 0.0, "min")
...
for step, max_mode in enumerate(max_modes):
    ...
    if QA_or_QH=='QA': opt_tuple.append((vmec.mean_iota, iota_target_QA, 1))
    if optimize_well: opt_tuple.append((penalty_magnetic_well.J, 0.0, 1))
    if optimize_DMerc: opt_tuple.append((penalty_DMerc.J, 0.0, 1))
    ...
    least_squares_mpi_solve(prob, mpi, grad=True,
                            ...
                            max_nfev=MAXITER)
    if extra_solve: least_squares_mpi_solve(prob, mpi, grad=True,
                            ftol=ftol, xtol=xtol,
                            abs_step=abs_step/10,
                            rel_step=rel_step/10,
                            diff_method=diff_method,
                            max_nfev=MAXITER)
