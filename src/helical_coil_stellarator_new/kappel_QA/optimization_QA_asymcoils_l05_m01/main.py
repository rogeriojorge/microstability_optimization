...
maxmodes_mpol_mapping             = {1: 3,  2: 3,  3: 3,  4: 3}
MAXITER_single_stage_mpol_mapping = {1: 15, 2: 15, 3: 15, 4: 15}
MAXFEV_single_stage_mpol_mapping  = {1: 15, 2: 15, 3: 15, 4: 15}
quasisymmetry_weight_mpol_mapping = {1: 1e-6, 2: 1e-6, 3: 1e-6, 4: 1e-6}
...
directory += f'_l0{l0_coil}_m0{m0_coil}' if ncoils>0 else f'_QAlinked_l0{l0_coil}_m0{m0_coil}'
...
CC_WEIGHT = 3e+1  # Weight for the coil-to-coil distance penalty in the objective function
...
ARCLENGTH_WEIGHT = 1e-11  # Weight for the arclength variation penalty in the objective function
##########################################################################################
##########################################################################################
vmec_verbose = False
...
def make_helical_curve(minor_radius, phase_shift=0.0):
    curve = CurveHelical(nquadpoints, order_coils, m=m0_coil, ell=l0_coil, R0=R0, r=minor_radius)
    if phase_shift != 0.0:
        curve_dofs = np.zeros(curve.num_dofs())
        curve_dofs[0] = phase_shift
        curve.x = curve_dofs
    return curve
...
if ncoils == 0:
    order_coils = 6
    extra_coil_R1 = 1.5
    base_curves = [make_helical_curve(ro_coil)]
    coils = [Coil(base_curves[0], Current(1)*1.00e5)]
    if use_two_coils:
        base_curves += [make_helical_curve(1.4*ro_coil, phase_shift=np.pi)]
        coils += [Coil(base_curves[1], Current(-1)*1.00e5)]
elif ncoils == 1:
    base_curves = [make_helical_curve(ro_coil)]
    coils = [Coil(base_curves[0], Current(1)*1e5)]
elif ncoils==2:
    base_curves = [make_helical_curve(ro_coil),
                   make_helical_curve(1.4*ro_coil, phase_shift=np.pi)]
    coils = [Coil(base_curves[0], Current(1)*1e5),
