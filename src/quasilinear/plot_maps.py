...
name_map = 'out_map_nfp4_QH_initial'
name_map_location = 'out_map_location_nfp4_QH_initial'
...
def plot_and_save(save_name, data, xlabel, ylabel, clb_title, plotExtent=None):
    ...
    if clb_title==r'$\gamma$' and ylabel=='$a/L_T$':
        if plot_extent_fix_gamma: plt.clim(plot_gamma_min,plot_gamma_max)
    if clb_title==r'$\gamma$' and ylabel==r'$\alpha$':
...
csv_file = os.path.join(this_path, name_map, f'{name_map}.csv')
...
csv_file = os.path.join(this_path, name_map_location, f'{name_map_location}.csv')
...
plot_and_save(f'{name_map}_gs2_scan_gamma.pdf', growth_rate_array, '$a/L_n$', '$a/L_T$', r'$\gamma$', plotExtent=plotExtent)
print('max omega =', np.max(omega_array), ', min omega =', np.min(omega_array))
plot_and_save(f'{name_map}_gs2_scan_omega.pdf', omega_array, '$a/L_n$', '$a/L_T$', r'$\omega$', plotExtent=plotExtent)
print('max ky =', np.max(ky_array), ', min ky =', np.min(ky_array))
plot_and_save(f'{name_map}_gs2_scan_ky.pdf', ky_array, '$a/L_n$', '$a/L_T$', r'$k_y$', plotExtent=plotExtent)
print('max weighted gamma =', np.max(weighted_growth_rate_array), ', min weighted gamma =', np.min(weighted_growth_rate_array))
plot_and_save(f'{name_map}_gs2_scan_weighted_gamma.pdf', weighted_growth_rate_array, '$a/L_n$', '$a/L_T$', r'$\gamma/\langle k_{\perp}^2 \rangle$', plotExtent=plotExtent)
...
plot_and_save(f'{name_map_location}_gs2_scan_gamma.pdf', growth_rate_location_array, r'$s$', r'$\alpha$', r'$\gamma$', plotExtent=plotExtent_location)
print('max overall omega =', np.max(omega_location_array), ', min overall omega =', np.min(omega_location_array))
plot_and_save(f'{name_map_location}_gs2_scan_omega.pdf', omega_location_array, r'$s$', r'$\alpha$', r'$\omega$', plotExtent=plotExtent_location)
print('max overall ky =', np.max(ky_location_array), ', min overall ky =', np.min(ky_location_array))
plot_and_save(f'{name_map_location}_gs2_scan_ky.pdf', ky_location_array, r'$s$', r'$\alpha$', r'$k_y$', plotExtent=plotExtent_location)
...
plot_and_save(f'{name_map_location}_gs2_scan_weighted_gamma.pdf', weighted_growth_rate_location_array, r'$s$', r'$\alpha$', r'$\gamma/\langle k_{\perp}^2 \rangle$', plotExtent=plotExtent_location)